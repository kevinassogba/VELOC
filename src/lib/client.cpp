...
static inline bool
validate_name(const std::string &name) {
    std::regex e("[a-zA-Z0-9_\\.]+");
    return std::regex_match(name, e);
}

static void
launch_backend(const std::string &cfg_file) {
    ...
    if (system(command.c_str()) != 0)
        FATAL("cannot launch active backend for async mode, error: "
              << strerror(errno));
}

bool
client_impl_t::check_threaded() {
    bool threaded = cfg.get_bool("threaded", false);
    if (threaded) {
        int provided;
        MPI_Query_thread(&provided);
        if (provided != MPI_THREAD_MULTIPLE)
            FATAL("MPI threaded mode requested but not available, please use "
                  "MPI_Init_thread with the MPI_THREAD_MULTIPLE flag");
    }
    return threaded;
}
...
client_impl_t::client_impl_t(MPI_Comm c, const std::string &cfg_file)
    ...
    if (cfg.is_sync() || check_threaded()) {
        ...
        MPI_Comm_split(comm, provided == 0 ? 0 : MPI_UNDEFINED, rank,
                       &backends);
        if (provided == 0)
...
bool
client_impl_t::cleanup(const std::string &name) {
...
bool
client_impl_t::checkpoint_wait() {
    if (checkpoint_in_progress) {
        ERROR("need to finalize local checkpoint first by calling "
              "checkpoint_end()");
        return false;
    }
...
bool
client_impl_t::checkpoint_finished() {
    if (cfg.is_sync())
        return true;
    if (checkpoint_in_progress) {
        ERROR("need to finalize local checkpoint first by calling "
...
bool
client_impl_t::checkpoint(const std::string &name, int version) {
    return checkpoint_wait() && checkpoint_begin(name, version) &&
           checkpoint_mem(VELOC_CKPT_ALL, {}) && checkpoint_end(true);
}

bool
client_impl_t::checkpoint_begin(const std::string &name, int version) {
    if (checkpoint_in_progress) {
        ERROR("nested checkpoints not yet supported");
        return false;
    }
    if (!validate_name(name) || version < 0) {
        ERROR("checkpoint name and/or version incorrect: name can only include "
              "[a-zA-Z0-9_] characters, version needs to be non-negative "
              "integer");
        return false;
    }
    ...
    current_ckpt =
        command_t(rank, command_t::CHECKPOINT, version, name.c_str());
    checkpoint_in_progress = true;
    return true;
}

bool
client_impl_t::checkpoint_mem(int mode, const std::set<int> &ids) {
    if (!checkpoint_in_progress) {
    ...
    if (ckpt_regions.size() == 0) {
        ERROR("empty selection of memory regions to checkpoint, please check "
              "protection and/or selective checkpointing primitives");
        return false;
    }
    ...
    try {
        f.open(current_ckpt.filename(cfg.get("scratch")),
               std::ofstream::out | std::ofstream::binary |
                   std::ofstream::trunc);
        size_t regions_size = ckpt_regions.size();
        size_t header_size =
            sizeof(size_t) + regions_size * (sizeof(int) + sizeof(size_t));
        ...
        ERROR("cannot write to checkpoint file: " << current_ckpt
                                                  << ", reason: " << f.what());
        return false;
    }
    return true;
...
bool
client_impl_t::checkpoint_end(bool /*success*/) {
    if (aggregated) {
        long offset = 0,
             next_offset = file_size(current_ckpt.filename(cfg.get("scratch")));
        if (rank > 0)
            MPI_Recv(&offset, 1, MPI_LONG, rank - 1, 0, comm,
                     MPI_STATUS_IGNORE);
        next_offset += offset;
        if (rank + 1 < no_ranks)
        ...
        if (rank == 0) {
            ...
            MPI_Gather(&offset, 1, MPI_LONG, &offset_map[1], 1, MPI_LONG, 0,
                       comm);
            if (!write_file(current_ckpt.agg_filename(cfg.get("meta")),
                            (unsigned char *) offset_map,
                            sizeof(long) * (no_ranks + 1)))
                return false;
        } else
...
int
client_impl_t::run_blocking(const command_t &cmd) {
    queue->enqueue(cmd);
    return queue->wait_completion();
}

int
client_impl_t::restart_test(const std::string &name, int needed_version) {
    ...
    int version = run_blocking(
        command_t(rank, command_t::TEST, needed_version, name.c_str()));
...
std::string
client_impl_t::route_file(const std::string &original) {
...
bool
client_impl_t::restart(const std::string &name, int version) {
    return restart_begin(name, version) && recover_mem(VELOC_CKPT_ALL, {}) &&
           restart_end(true);
}

bool
client_impl_t::restart_begin(const std::string &name, int version) {
    if (checkpoint_in_progress) {
...
bool
client_impl_t::read_current_header() {
    header_size =
        read_header(current_ckpt.filename(cfg.get("scratch")), region_info);
    return header_size != 0;
}

size_t
client_impl_t::recover_size(int id) {
    if (header_size == 0)
        read_current_header();
...
bool
client_impl_t::recover_mem(int mode, const std::set<int> &ids) {
    if (header_size == 0 && !read_current_header()) {
        ERROR(
            "cannot recover in memory mode if header unavailable or corrupted");
        return false;
    }
    try {
        std::ifstream f;
        ...
        for (auto &e : region_info) {
            ...
            if ((mode == VELOC_RECOVER_SOME && !found) ||
                (mode == VELOC_RECOVER_REST && found)) {
            ...
            if (info.ptr != NULL) {   // direct read of raw data
                if (info.size < e.second) {
                    ERROR("protected memory region "
                          << e.first << " is too small (" << info.size
                          << ") to hold required size (" << e.second << ")");
                    return false;
                }
                f.read((char *) info.ptr, e.second);
            } else {   // deserialize
                if (!info.d(f)) {
                    ERROR("protected data structure "
                          << e.first << " could not be deserialized");
                    return false;
                }
            }
        }
...
bool
client_impl_t::restart_end(bool /*success*/) {
    return true;
}
